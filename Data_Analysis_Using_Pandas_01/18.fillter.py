...
total_salary_by_dep = '''
        SELECT 
            Department,
            SUM(Salary) AS Total_Salary_by_dep
        FROM df
        GROUP BY Department
        ORDER BY Total_Salary_by_dep DESC
'''
sqldf(total_salary_by_dep, locals())
...
'''
sqldf(sort_by_age_salary,locals())
...
dep_wise_emp_count = '''
        ...
        ORDER BY 2 DESC
'''
sqldf(dep_wise_emp_count,locals())

df.groupby(['Department']).agg({"Emp_No":"count"})

# Task 15: Filter Employees with the letter o in the Name.
df.info()
emp_with_latter_O = '''
        SELECT * FROM df
        WHERE LOWER(Emp_Name) LIKE "%o%"
'''
sqldf(emp_with_latter_O,locals())

df[df['Emp_Name']=='sonam']


df['Emp_No'] = df['Emp_No'].astype(str)


df[df['Emp_No'].str.contains('o')]
