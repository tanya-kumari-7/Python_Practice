...
def view_all_task():
    with open("text_book.txt", "r") as file:
        ...
        return [line.strip() for line in file_details]
   
# view_all_task()

def view_task_by_id(id):
    if not id:
        return "Please enter your task id"
    case_status = False
    ...
    with open("text_book.txt", "r") as file:
        ...
        for line in file_details:
            ...
            if task_id == input_id:
                ...
                return line.strip()
    if not case_status:
        return f"{input_id} : Id not found, please enter valid id or please add task with this id [If ANY]"
...
def delete_task_by_id(task_id):
    ...
    if case_status:
        return f"Task {input_task_id} deleted."
    else:
        return f"{input_task_id} not found in the book."

# delete_task_by_id(20)

def update_status_date_by_id (id,updated_status,date):
    if not updated_status or not date or not id:
        return "Please enter status which need to be updated and date"
    ...
    if case_status:
        ...
        return f"{input_id} id not found in the book, please re-check"
...
def month_wise_task_summary():
    ...
    summary_dict = month_wise_task_summary_.to_dict()
    return {f"{k[0]} - {k[1]}": v for k, v in summary_dict.items()}
